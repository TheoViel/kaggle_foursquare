...
def load_encoder(model, filename, verbose=1, cp_folder=""):
    """
    Loads the weights of a pretrained encoder.
    ...
    """
    if verbose:
        print(f"\n -> Loading backbone from {os.path.join(cp_folder, filename)}\n")

    dic = torch.load(os.path.join(cp_folder, filename), map_location="cpu")

    dic = {k.replace('encoder.', ''): dic[k] for k in dic if "encoder" in k}

    model.encoder.load_state_dict(dic, strict=True)
