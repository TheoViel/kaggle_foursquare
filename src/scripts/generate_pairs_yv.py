...
# read data
train = pd.read_csv("../data/train.csv")  # .head(10000)
...
Potential_on_NamePhone = work.apply(
    lambda row: find_potential_matchs(row), axis=1
).to_list()
...
p1.to_csv("P1_CHECK.csv", index=False)
p2.to_csv("P2_CHECK.csv", index=False)
