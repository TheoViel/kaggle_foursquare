...
# Vincent : Mise à jour de F_SKIP0
...
'zip_lcs2_r1','url_pi1_r1','zip_dsm1','zip_lllcs_r2','zip_lcs_r2','phone_ll1','url_lcs_r1','phone_lcs','zip_lcs2_r2','address_m5','city_lllcs_r1','zip_lcs2', 'state_group_m5',
'city_group_cclcs', 'city_group_m10','state_group_cclcs','state_group_ll1','state_group_lllcs_r1','state_group_m10','city_group_m5','state_group_lllcs','state_group_lllcs_r2','city_group_lllcs_r1','state_group_pi2','state_group_lcs2','state_group_pi1','city_group_lcs_r4','city_group_pi2_r1','state_group_pi1_r1',
'state_group_lcs_r4','state_group_lcs_r1','city_group_lcs_r1','state_group_lcs','city_group_lllcs_r2','state_lcs_r1','state_group_pi2_r1','city_group_lcs2', 'state_group_dsm2','state_group_lcs2_r2','city_group_lllcs','state_group_ljw','state_group_lcs2_r1',
'city_group_pi2','state_group_lcs_r2'])
...
def FE2(df, p1, p2, train, ressources_path=""):
    ...
    # Vincent : add features. If it's not working (Nb_multiPoi not in df, just skip)
    eps, MAX = 1e-1, 1e5
    ...
    df["ratio_connect_multipoi1"] = df["ratio_connect_multipoi1"].apply(lambda x : x if x <= MAX else MAX).astype('float32')
    df["ratio_connect_multipoi2"] = df["ratio_connect_multipoi2"].apply(lambda x : x if x <= MAX else MAX).astype('float32')
    ...
    # Avoid too high values
    df["ratio_strong_connect_multipoi1"] = df["ratio_strong_connect_multipoi1"].apply(lambda x : x if x <= MAX else MAX).astype('float32')
    df["ratio_strong_connect_multipoi2"] = df["ratio_strong_connect_multipoi2"].apply(lambda x : x if x <= MAX else MAX).astype('float32')

    del Connexes, graph  # , Len_strong_connect
    gc.collect()

    ################################
    #  Cat link score (Vincent)
    ...
    # Vincent : add city_group and state_group
    for col in ["name", "address", "categories", "id", "city", "state", "city_group", "state_group", "zip", "phone"]:
        p12 = p1[[col]].append(
            p2[[col]], ignore_index=True
        )  # count it in both p1 and p2
        df1 = p12[col].value_counts()
    ...
    #######################################################
    # Vincent : Features related to number in names
